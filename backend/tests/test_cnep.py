...
print("Consultando CNEP - CNPJ: " + CNPJ)
print("-" * 60)
...
try:
    ...
    if r["possui_sancao"]:
        print("ALERTA: " + str(r["total_registros"]) + " registro(s) CNEP")
        print(json.dumps(r["registros"][0], ensure_ascii=False, indent=2))
    else:
        print("OK: Nenhuma sancao CNEP encontrada")

    out = Path(__file__).parent / "output_cnep.json"
